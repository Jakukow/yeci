import { USDC_MAIN } from "../../consts/static";
  ...
  return (
    <ModalWrapper show={show} onClose={handleClose} className="w-[480px]">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-cyan-300/60">Enter amount to deposit</p>
      </div>

      <div className="mb-6 p-4 rounded-xl bg-cyan-950/30 border border-cyan-500/20">
        <div className="flex justify-between items-center">
          <span className="text-cyan-300/70 text-sm">Wallet Balance</span>
          <span className="text-cyan-100 font-semibold">
            {printBN(availableBalance ?? new BN(0), USDC_MAIN.decimals)} USDC
          </span>
        </div>
      </div>
      ...
      <button
        ...
        disabled={
          !depositAmount || parseFloat(depositAmount) <= 0 || depositOngoing
        }
